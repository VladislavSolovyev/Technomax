...
class Draw():
    ...
    @staticmethod
    def window(ar, figures, area, conveyors):
        ...
        for i, conveyor in enumerate(conveyors):
            for coord in conveyor:
                c.create_rectangle(coord.y * koef, coord.x * koef, (coord.y + 1) * koef, (coord.x + 1) * koef,
                                   fill="#463E3F", width=3)
                c.create_text((coord.y + 0.5) * koef, (coord.x + 0.5) * koef, text=str(i + 1), font="Verdana 10",
                              fill="white")
